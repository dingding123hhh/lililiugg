...
game:GetService("Workspace").Stores.WoodRUs.Parts.SELLWOOD.SurfaceGui.TextLabel.Text = "禁漫中心";
game:GetService("Workspace").Stores.WoodRUs.Parts.SELLWOOD.SurfaceGui.TextLabel.TextColor3 = Color3.fromRGB(0, 248, 255)
game:GetService("Workspace").Stores.WoodRUs.Parts.WOODDROPOFF.SurfaceGui.TextLabel.Text = "禁漫中心";
game:GetService("Workspace").Stores.WoodRUs.Parts.WOODDROPOFF.SurfaceGui.TextLabel.TextColor3 =
    Color3.fromRGB(0, 248, 255)
    ...
    if v.Name ~= "Chat" and v.Name ~= "TargetGui" then
        for i, v in next, v:GetDescendants() do
            ...
            if v:IsA("TextButton") or v:IsA("Frame") or v:IsA("ScrollingFrame") then
                v.BackgroundColor3 = Color3.fromRGB(0, 248, 255)
            end
...
local function donate(plr, amount)
    ...
    notify('丁丁', '转钱' .. tostring(amount) .. ' 给 ' .. tostring(plr), 4);
    scr2.exitNotice();
end
...
about:Button("锯木机最大木头体型",function()
...
about:Button("启动所有压力板",function()
    burnAllShopItems()
end)

about:Button("船离开时间",function()

    if game.Workspace.Ferry.TimeToDeparture.Value == 0 then
        notify("丁丁", "船已离开", 4)
    else
        notify("丁丁", "距离船离开时间有" .. game.Workspace.Ferry.TimeToDeparture.Value .. "秒", 4)
    end
end)

about:Button("桥上升时间",function()

    if game.Workspace.Bridge.DownTime.Value < 0 then
        notify("丁丁", "桥已上升", 4)
    else
        notify("丁丁", "距离桥上升的时间有" .. game.Workspace.Bridge.DownTime.Value .. "秒", 4)
    end
end)
...
about:Button("关/开家具店的门",function()
...
about:Button("打开鲨鱼斧合成地方的门",function()
...
about:Button("删除鲨鱼斧合成地方的门",function()

    local door = game:GetService('Workspace')['Region_Snow'].Den.Hatch;
    door:Destroy()

end)
about:Button("关/开连锁逻辑店的门",function()
...
about:Button("关/开盒子车行的门",function()
...
about:Button("汽车传送到玩家旁边",function()
    local tp_player = game:GetService("Players")[Green.playernamedied]
...
about:Button("汽车传送到玩家基地",function()
...
about:Button("重进服务器",function()
    game:GetService("TeleportService"):Teleport(13822889)
end)

about:Button("拿出可口可乐",function()
