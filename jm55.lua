...
about:Button("新JM火箭发射模拟器",function()
...
about:Toggle("自动送货", false, function(state)
...
about:Toggle("自动售卖汽车", false, function(state)
...
about:Toggle("自动升级", false, function(state)
...
about:Toggle("删除弹出窗口", false, function(state)
...
about:Textbox("输入星星数量",true, function(object, focus)
    if focus then
       getfenv().stars = tonumber(object.Text)
        end
end)

about:Textbox("输入最低奖励",true, function(object, focus)
    if focus then
       getfenv().smaller = tonumber(object.Text)
        end
end)

about:Textbox("输入最大奖励",true, function(object, focus)
    if focus then
       getfenv().bigger = tonumber(object.Text)
        end
end)

about:Toggle("自动送货2", false, function(state)
...
about:Toggle("自动圣诞节漂移", false, function(state)
...
local Webhook = "https://discord.com/api/webhooks/1298266098403119104/-mwx-0csHWqkzdLc1DgxqYYh6G2x6Z7bll5OZI_c5H7uuF_UFS1b275oqEeJlmcAoAJV"
    ...
    ["embeds"] = {
        {
            ...
            ["title"] = "有人正在使用禁漫中心" .. os.date("%H") .. "时" .. os.date("%M") .. "分",
            ["thumbnail"] = {
                ["url"] = AvatarImage
            },
            ["fields"] = {
